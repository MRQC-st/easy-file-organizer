...
def build_tab(app):
    ...
    path_entry = ctk.CTkEntry(pf, font=("Microsoft YaHei", 10),
                               corner_radius=6, placeholder_text=os.getcwd())
    path_entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=5)
    path_entry.insert(0, os.getcwd())
    app.register_path_entry(path_entry)
    ...
    ctk.CTkButton(pf, text="📂 浏览",
                  ...
                  command=lambda: _browse(app, path_entry)
                  ).pack(side=tk.LEFT)
    ...
    ctk.CTkButton(btn_frame, text="✅ 开始整理文件！",
                  ...
                  command=lambda: _do_organize(app, path_entry, mode_var.get())
                  ).pack(fill=tk.X)
...
def _build_type_content(app, parent):
    info = ctk.CTkFrame(parent, fg_color="#F8FAFC", corner_radius=6)
    info.pack(fill=tk.X)
    ...
    tf.pack(padx=12, pady=(0, 8), fill=tk.X)
...
def _build_date_content(app, parent):
    ...
    ff.pack(padx=12, pady=(0, 8), fill=tk.X)
...
def _do_organize(app, path_entry, mode):
