...
# ── 高 DPI 适配：避免 Windows 缩放导致模糊 ──
try:
    from ctypes import windll
    windll.shcore.SetProcessDpiAwareness(2)  # Per-Monitor DPI 感知
except Exception:
    try:
...
class App(ctk.CTk):
    def __init__(self):
        ...
        self._path_entries = []
        self.current_module = tk.StringVar(value="")
    ...
    def _build_ui(self):
        ...
        self.grid_rowconfigure(0, weight=0)  # 标题栏固定高度
        self.grid_rowconfigure(1, weight=1)  # 主内容区可扩展
        self.grid_columnconfigure(0, weight=1)

        # 顶部标题栏
        top_bar = ctk.CTkFrame(self, fg_color="white", height=44, corner_radius=0)
        ...
        self.left_frame.grid(row=0, column=0, sticky="nsew", padx=(0, 5), pady=10)

        # 右侧操作面板 — 内部内容可滚动
        self.right_frame = ctk.CTkFrame(main, fg_color="white", corner_radius=8)
        self.right_frame.grid(row=0, column=1, sticky="nsew", padx=(5, 0), pady=10)

        # 底部日志区
        log_outer = ctk.CTkFrame(self, fg_color=self.COLORS["bg_main"], height=110)
        log_outer.grid(row=2, column=0, sticky="ew", padx=10, pady=(0, 6))
        log_outer.pack_propagate(False)

        # 底部日志
        self._log_parent = log_outer
        self._build_log()

    def _build_log(self):
        log_frame = ctk.CTkFrame(self._log_parent, fg_color="white", corner_radius=8)
        ...
        self.processing_label = ctk.CTkLabel(header, text="",
                                             font=("Microsoft YaHei", 10),
                                             text_color=self.COLORS["warning"])
        self.processing_label.pack(side=tk.RIGHT, padx=14)
        ...
        self.log_text = ctk.CTkTextbox(log_frame,
                                       ...
                                       corner_radius=6)
        self.log_text.pack(fill=tk.BOTH, expand=True, padx=10, pady=(0, 6))

    def set_processing(self, msg="⏳ 处理中..."):
        self.processing_label.configure(text=msg)
    ...
    def _build_preview_area(self):
        ...
        header.pack(fill=tk.X, padx=10, pady=(14, 6))
        ...
        path_frame.pack(fill=tk.X, padx=10, pady=(4, 8))
        ...
        ctk.CTkButton(path_frame, text="🔄 刷新",
                      font=("Microsoft YaHei", 10),
    ...
    def _build_drop_zone(self, parent):
        ...
        drop_outer.pack(fill=tk.X, padx=10, pady=(0, 8))

        self.drop_canvas = tk.Canvas(drop_outer, height=110,
    ...
    def _build_file_type_bar(self, parent):
        ...
        bar_outer.pack(fill=tk.X, padx=0, pady=(0, 4))
    ...
    def _build_action_panel(self):
        ...
        cards_outer.pack(fill=tk.X, padx=10, pady=(6, 3))

        self.card_buttons = {}
        self._prev_card_module = None
        ...
        rule_header.pack(fill=tk.X, padx=10, pady=(3, 2))
        ...
        # 规则内容区
        self.rule_frame = ctk.CTkFrame(panel, fg_color="transparent")
        self.rule_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=(0, 4))
    ...
    def _create_card(self, parent, icon_key, text, module):
        card = ctk.CTkFrame(parent, fg_color="white", corner_radius=8,
                            ...
                            cursor="hand2")
        card.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=3, pady=1)

        icon_ctk = self.icons.get(icon_key)
        if icon_ctk:
            icon_lbl = ctk.CTkLabel(card, image=icon_ctk, text="",
                                    fg_color="transparent")
            icon_lbl.pack(pady=(3, 1))
        else:
            icon_lbl = ctk.CTkLabel(card, text="📁",
                                    font=("Microsoft YaHei", 20),
                                    fg_color="transparent")
            icon_lbl.pack(pady=(3, 1))

        lbl = ctk.CTkLabel(card, text=text, font=("Microsoft YaHei", 9),
                           text_color=self.COLORS["text_primary"],
                           fg_color="transparent")
        lbl.pack(pady=(0, 3))
    ...
    def _switch_module(self, module):
        current = self.current_module.get() if hasattr(self.current_module, 'get') else self.current_module
        if module == current:
            return
        # 清除内容并重建
    ...
    def _highlight_card(self, module):
        # 只更新变化的卡片，不遍历全部
        prev = self._prev_card_module
        if prev and prev in self.card_buttons:
            card, lbl = self.card_buttons[prev]
            card.configure(border_color=self.COLORS["border"], fg_color="white")
            lbl.configure(text_color=self.COLORS["text_primary"])
        if module in self.card_buttons:
            card, lbl = self.card_buttons[module]
            card.configure(border_color=self.COLORS["primary"], fg_color="#EFF6FF")
            lbl.configure(text_color=self.COLORS["primary"])
        self._prev_card_module = module
    ...
    def refresh_folder(self):
        folder = self.path_display.get().strip()
        if not folder:
            return messagebox.showwarning("提示", "请先选择文件夹")
        self.load_folder(folder)
