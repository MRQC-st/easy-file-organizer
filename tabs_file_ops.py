...
def build_tab(app):
    ...
    ctk.CTkLabel(pf, text="当前文件夹：", font=("Microsoft YaHei", 10),
    ...
    _make_ops_module(app, "📦 批量移动文件",
                     ...
                     lambda: _batch_move(app, path_entry))
    ...
    _make_ops_module(app, "📋 批量复制文件",
                     ...
                     lambda: _batch_copy(app, path_entry))
...
def _batch_move(app, path_entry):
...
def _batch_copy(app, path_entry):
    src = path_entry.get().strip()
    if not src or not os.path.isdir(src):
        return messagebox.showwarning("提示", "请先选择有效的源文件夹")
    dst = filedialog.askdirectory(title="选择目标文件夹")
